<section class="bg-neutral text-neutral-content rounded-3xl">
    <div class="max-w-7xl mx-auto px-8 py-8 md:py-32 text-center">
        ...
        <div class="flex flex-col md:flex-row justify-center items-center md:items-start gap-6">
            @foreach ($steps as $index => $step)
                <div class="w-full md:w-48 flex flex-col gap-2 items-center justify-center">
                    ...
                    <h3 class="font-bold">{{ $step['text'] }}</h3>
                </div>

                @if ($index < count($steps) - 1)
                    <svg class="shrink-0 w-12 fill-neutral-content opacity-70 {{ $index % 2 === 0 ? 'max-md:-scale-x-100 md:-rotate-90' : 'md:-scale-x-100 md:-rotate-90' }}" viewBox="0 0 138 138" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g>
